...
/* ============================================================
   开屏起点：等字体就绪
   ------------------------------------------------------------
   要解决的问题：WELCOME 和正文都用 HuXiaoBo。原来所有开屏计时都从
   「页面加载」开始算，而字体往往要晚一两秒才到 —— 结果那行巨大的
   WELCOME 先用替代字体渲染，等字体到了再当着用户的面重排一次。
   ...
   ★ P27：这条闸门当初是为 3.2MB 的 HuXiaoBo.otf 设的。现在正文走的是
     子集化后的 HuXiaoBo-subset.woff2（30KB，原体积的 0.96%），几乎瞬间
     到位 —— 闸门照样在，但正常情况下几乎不会真的"等"。
     于是 MAX_WAIT 变成了一条纯粹的兜底：只有在 30KB 都下不动的极端
     网络下才会被触发。想收紧的人可以放心往 800~1000 调。
   ============================================================ */
window.Intro = (function introGate() {
    ...
    if (!canWait) {
        ...
        /* 等开屏真正用到的字体，并且和超时赛跑。
           ★ P27：这里原来还等 'Crunch-Light'（数字时钟的字体）。数字时钟在
             P26 已经整个移除，那条 load() 就变成「等一个全站没有任何元素
             使用的字体」—— 它会真的把 69KB 拉下来，和正文抢带宽，
             白白把开屏闸门往后拖。已删。
           ★ P27：正文那份现在等的是子集化后的 'HuXiaoBo'（30KB）。 */
        Promise.all([
